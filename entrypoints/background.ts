...
// NOTE: the content script already sends the canonical lemma (the exact value
// analyzeTokens produced). It must be stored VERBATIM — re-lemmatizing it here
// is NOT idempotent for Polish (a lemma-form can map to a different base), which
// would file the word under a key that no page token ever resolves to.
...
async function setWordStatusImpl(payload: Extract<Message, { type: 'SET_WORD_STATUS' }>['payload']): Promise<{ ok: true }> {
  const { lang, status, lemma } = payload
  const statuses = await statusMapFor(lang)
  if (status === 'unknown') {
...
async function setWordTranslationImpl(payload: { lang: string; lemma: string; translation: string }): Promise<{ ok: true }> {
  const { lang, translation, lemma } = payload
  const statuses = await statusMapFor(lang)
  const existing = await getWord(lang, lemma)
  const now = Date.now()
    ...
    try {
      switch (message.type) {
        ...
        case 'GET_WORD_TRANSLATION': {
          const { lang, lemma } = message.payload
          const rec = await getWord(lang, lemma)
